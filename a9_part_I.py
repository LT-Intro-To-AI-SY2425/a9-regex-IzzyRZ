...
pat = re.compile("(\w+)\s+(\d+),\s+(\d{4})", re.IGNORECASE)
...
pat = re.compile("(?P<number>\d+)\s+(?P<street>[\w\s]+)[\s\n]+(?P<city>[\w\s]+),\s+(?P<state>\w{2})\s+(?P<zip>\d{5})", re.IGNORECASE)
